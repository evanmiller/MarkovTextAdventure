# I don't know Julia. But I do know the MarkovTextAdventure.jl wasn't
# running on my Debian Stable machine (Julia version 0.3.2). So, I
# copied it to this file, tweaked it, and it (mostly) runs now.
# However, I'm not going to overwrite the original file, since, like I
# said, I don't know Julia. I just frobbed things until it worked.
...
function fill_transition_matrix(matrix::AbstractArray{Float64,2}, bad_design::Bool, indexes::Array{Integer})
    ...
    for i=1:(size(matrix,1)-1)
        ...
        for j=1:length(actions)
            if actions[j][1].op == OP_FINISH
                matrix[i,size(matrix,2)] += 1.0/length(actions)
            else
                new_state = apply_action(state, actions[j][1], bad_design)
...
function analyze_game(bad_setup::Bool, bad_move::Bool)
    ...
    fill_transition_matrix(transition_matrix2, bad_move, initial_states[1])
    ...
    if ev1 == ev2
        ...
        println(" => There is a dead end :-( ", ev1, " != ", ev2)
...
#    revalues, revectors = eig(transition_matrix1; scale=false)
    revalues, revectors = eig(transition_matrix1)
